...
from ghmarkdown.label import MhLabels
                ...
                for tag in sorted(self.mappingTags(self.tags, self.allTags)):
                    ...
                    if len(content["title"]) > 0:
                        MhReportEntry(content, self.filteredFiles.copy(), self.allTags,
                                      self.allSubContents, self.commentTag, self.showTags, self.labels,
                                      self.level).generate(writer)
            # Proceed to else of VIRTUAL block
            try:
                MhReportEntry(self.json["else"], self.elseFiles, self.allTags,
                              self.allSubContents, self.commentTag, self.showTags, self.labels, self.level).generate(
                    writer)
            except KeyError:
                pass
                ...
                if json_count is not None:
                    ...
                    for name, file in self.filteredFiles.items():
                        ...
                        if self.showTags is not None:
                            for showTag in self.showTags:
                                for tag in file.getTagStartingBy(showTag):
                                    ...
                                    if len(stag) > 0:
                                        commentTags = "{} ``{}``".format(commentTags, stag)
                        # Main line with entry found data
                        #                    writer.writelines("- [[{}]] {} {} \n".format(name, commentTags, comment))
                        ...
                        if self.commentTag is not None:
                            ...
                            writer.writelines("[[{}]]  {} \n".format(name, commentTags))

            try:
                MhReportEntry(self.json["else"], self.elseFiles, self.allTags,
                              self.allSubContents, self.commentTag, self.showTags, self.labels, nextLevel).generate(
                    writer)
            except KeyError:
                pass
